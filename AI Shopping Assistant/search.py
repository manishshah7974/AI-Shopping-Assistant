...
class ShoppingAssistant:
    ...
    def chat(self, query: str, top_k: int = 5) -> str:
        ...
        prompt = f"""You are a helpful shopping assistant for a beauty & fragrance e-commerce store.
...
Response:"""
    ...
    def compare(self, product_a: str, product_b: str) -> str:
        ...
        prompt = f"""You are a shopping assistant. Compare these two product groups concisely.
    ...
    def _format_products(self, results: List[Dict]) -> str:
        ...
        for i, r in enumerate(results, 1):
            ...
            lines.append(f"{i}. {m['name']} | Brand: {m['brandName']} | "
                         f"Price: {m['sellingPrice']}{discount} | Gender: {', '.join(m['gender'])}")
        return "\n".join(lines)
